...
```{tip}
The "20" prefix refers to the snap bases that snaps are based on, in this case we use `20` for branches using to `base: core20` which in turn refers to Ubuntu 20.04LTS.
```
...
```
git diff 20/main 20/x11-glxgears
```
...
snap install ubuntu-frame --channel 20
