...
These interfaces are an evolution of {ref}`the-graphics-core20-snap-interface` - if your application uses `base: core20`, you'll need to rely on that, or move to the newer base.
...
3. use [`bin/gpu-2404-wrapper`](https://github.com/canonical/gpu-snap/blob/main/bin/gpu-2404-wrapper) in your <code>{doc}`command-chain <snapcraft:reference/project-file/snapcraft-yaml>`</code>s:
...
If your snap currently uses the `graphics-2404` interface, here are the steps when migrating:
