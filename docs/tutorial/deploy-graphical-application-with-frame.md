...
Ubuntu Frame has various channels corresponding to the snap bases, we use `--channel=22` here which corresponds to `base: core22` which in turn refers to Ubuntu 22.04 LTS.
...
```{tip}
The "22" prefix refers to the snap bases, in this case we use `22/` for branches using to `base: core22`.
```
