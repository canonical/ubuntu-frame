    ...
    return runner.run_with(
        {
            ...
            ConfigurationOption{[&] (auto& option) { background_client.set_font_path(option);},
                               "diagnostic-font-path", "Path (including filename) of font", "/usr/share/fonts/truetype/ubuntu/Ubuntu-R.ttf"},
            ...
            ConfigurationOption{[&] (auto& option) { background_client.set_y_margin(option);},
                               "diagnostic-y-margin",     "Size of x-axis margin (in percentage of display height) for diagnostic", "5"},
